...
class Api {
  ...
  * @param to
  * @param amount
  * @returns {Object} (block_num 操作所属块号, txid 操作id) | (e 异常信息)
  */
  async collectCsaf(from, to, amount) {  
    try {
      let time_point_sec = parseInt( parseInt( new Date().getTime() / 1000 ) / 60 ) * 60;
      let op_data = {
        from: from,
        to: to,
        amount: {amount: parseInt(amount * 1000), asset_id: 0},
        time: time_point_sec
      };
      let tr = new TransactionBuilder();
      tr.add_type_operation('csaf_collect', op_data);
      await tr.set_required_fees(from, false, true);
      if(from === to){
        tr.add_signer(PrivateKey.fromWif(config.secondary_key));
      }else{
        tr.add_signer(PrivateKey.fromWif(config.active_key));
      }
      return await this.__broadCast(tr);
    } catch (e) {
      return {
